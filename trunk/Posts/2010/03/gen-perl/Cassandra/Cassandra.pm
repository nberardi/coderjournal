      ...
      /^0$/ && do{      if ($ftype == TType::MAP) {
        {
          ...
          {
            my $key40 = '';
            my $val41 = new Cassandra::ColumnOrSuperColumn();
            $xfer += $input->readString(\$key40);
            $val41 = new Cassandra::ColumnOrSuperColumn();
      ...
      /^0$/ && do{      if ($ftype == TType::MAP) {
        {
          ...
          for (my $_i55 = 0; $_i55 < $_size51; ++$_i55)
          {
            ...
            $xfer += $input->readString(\$key56);
            {
              ...
              {
                my $elem63 = undef;
                $elem63 = new Cassandra::ColumnOrSuperColumn();
    ...
    $xfer += $output->writeFieldBegin('success', TType::MAP, 0);
    {
      $output->writeMapBegin(TType::STRING, TType::LIST, scalar(keys %{$self->{success}}));
      {
        while( my ($kiter64,$viter65) = each %{$self->{success}}) 
        {
          ...
          {
            $output->writeListBegin(TType::STRUCT, scalar(@{${viter65}}));
            {
...
package Cassandra::Cassandra_get_key_range_args;
use base qw(Class::Accessor);
Cassandra::Cassandra_get_key_range_args->mk_accessors( qw( keyspace column_family start finish count consistency_level ) );
...
package Cassandra::Cassandra_get_key_range_result;
use base qw(Class::Accessor);
Cassandra::Cassandra_get_key_range_result->mk_accessors( qw( success ) );
    ...
    $xfer += $output->writeFieldBegin('success', TType::MAP, 0);
    {
      $output->writeMapBegin(TType::STRING, TType::MAP, scalar(keys %{$self->{success}}));
      {
        while( my ($kiter118,$viter119) = each %{$self->{success}}) 
        {
          ...
          {
            $output->writeMapBegin(TType::STRING, TType::STRING, scalar(keys %{${viter119}}));
            {
...
sub send_get_key_range{
  ...
  $self->{output}->writeMessageBegin('get_key_range', TMessageType::CALL, $self->{seqid});
  my $args = new Cassandra::Cassandra_get_key_range_args();
  $args->{keyspace} = $keyspace;
...
sub recv_get_key_range{
  ...
  my $result = new Cassandra::Cassandra_get_key_range_result();
  $result->read($self->{input});
  $self->{input}->readMessageEnd();
...
sub process_get_key_range {
    ...
    $output->writeMessageBegin('get_key_range', TMessageType::REPLY, $seqid);
    $result->write($output);
    $output->writeMessageEnd();
    $output->getTransport()->flush();
}
