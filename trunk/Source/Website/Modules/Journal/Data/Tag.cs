/*
 * 	Template:		This code was generated by the Coder Journal [http://www.coderjournal.com] Data Layer Template.
 * 	Created On :	8/27/2006
 * 	Remarks:		Changes to this file may cause incorrect behavior and will be lost if the code is regenerated.
 */

using System;
using System.Collections;
using System.Collections.Generic;
using System.ComponentModel;
using System.Configuration;
using System.Data;
using System.Data.SqlClient;
using System.Data.SqlTypes;
using System.Diagnostics;
using System.Text;
using System.Threading;
using System.Security.Permissions;
using System.Xml;
using System.Xml.Serialization;

namespace CoderJournal.Modules.Journal.Data
{
	[DataObject(true)]
	public partial class Tag : ITable<int>
	{
		#region Static Methods
		
		#region Common Methods

		protected static TagCollection FillCollection (SqlCommand command)
		{
			TagCollection list = new TagCollection();
			
			try
			{
				command.Connection.Open();
				using(SqlDataReader reader = command.ExecuteReader())
				{
					if (reader.HasRows)
					{
						int[] order = new int[4];
						order[0] = reader.GetOrdinal("TagID");
						order[1] = reader.GetOrdinal("Name");
						order[2] = reader.GetOrdinal("Description");
						order[3] = reader.GetOrdinal("SortOrder");

						while (reader.Read()) 
						{
							Tag entity = new Tag();
							entity._tagID = reader.IsDBNull(0) ? 0 :  reader.GetInt32(order[0]); // TagID
							entity._name = reader.IsDBNull(1) ? String.Empty :  reader.GetString(order[1]); // Name
							entity._description = reader.IsDBNull(2) ? (string)null :  reader.GetString(order[2]); // Description
							entity._sortOrder = reader.IsDBNull(3) ? 0 :  reader.GetInt32(order[3]); // SortOrder

							// add to list
							list.Add(entity);
						}
					}
				}
			} catch (Exception exc) {
				Debug.WriteLine(exc);
			} finally {
				command.Connection.Close();
			}
					
			return list;
		}
	
		protected static Tag FillEntity (SqlCommand command)
		{
			Tag entity = null;
			
			try
			{
				command.Connection.Open();
				using(SqlDataReader reader = command.ExecuteReader())
				{
					if (reader.HasRows)
					{
						reader.Read();
						entity = new Tag();
						entity._tagID = reader.IsDBNull(0) ? 0 :  reader.GetInt32(reader.GetOrdinal("TagID"));
						entity._name = reader.IsDBNull(1) ? String.Empty :  reader.GetString(reader.GetOrdinal("Name"));
						entity._description = reader.IsDBNull(2) ? (string)null :  reader.GetString(reader.GetOrdinal("Description"));
						entity._sortOrder = reader.IsDBNull(3) ? 0 :  reader.GetInt32(reader.GetOrdinal("SortOrder"));
					}
				}
			} catch (Exception exc) {
				Debug.WriteLine(exc);
			} finally {
				if (entity == null) 
					entity = new Tag();
					
				command.Connection.Close();
			}
					
			return entity;
		}
		
		#endregion
		
		#region Get List
		
		[DataObjectMethod(DataObjectMethodType.Select, false)]
		public static TagCollection GetList (string where, string orderBy)
		{
			StringBuilder sb = new StringBuilder(10);
			
			sb.Append(@"select * from [Tag] ");
			
			if (String.IsNullOrEmpty(where) == false)
			{
				sb.Append(" where ");
				sb.Append("(");
				sb.Append(where);
				sb.Append(")");
			}
			
			if (String.IsNullOrEmpty(orderBy) == false)
			{
				sb.Append(" order by ");
				sb.Append(orderBy);
			}
			
			using(SqlConnection connection = new SqlConnection(ConfigurationManager.ConnectionStrings["ManagedFusion"].ConnectionString))
			{
				using (SqlCommand command = connection.CreateCommand())
				{
					command.CommandText = sb.ToString();
					command.CommandType = CommandType.Text;
					
					return FillCollection(command);
				}
			}
		}
		
		[DataObjectMethod(DataObjectMethodType.Select, false)]
		public static TagCollection GetList (string where)
		{
			return GetList(where, String.Empty);
		}
		
		[DataObjectMethod(DataObjectMethodType.Select, true)]
		public static TagCollection GetList ()
		{
			return GetList(String.Empty, String.Empty);
		}
		
		#endregion
		
		#region Get First
	
		[DataObjectMethod(DataObjectMethodType.Select, false)]
		public static Tag GetFirst (string where, string orderBy)
		{
			StringBuilder sb = new StringBuilder(10);
			
			sb.Append(@"select top 1 * from [Tag] ");
			
			if (String.IsNullOrEmpty(where) == false)
			{
				sb.Append(" where ");
				sb.Append("(");
				sb.Append(where);
				sb.Append(")");
			}
			
			if (String.IsNullOrEmpty(orderBy) == false)
			{
				sb.Append(" order by ");
				sb.Append(orderBy);
			}
			
			using(SqlConnection connection = new SqlConnection(ConfigurationManager.ConnectionStrings["ManagedFusion"].ConnectionString))
			{
				using (SqlCommand command = connection.CreateCommand())
				{
					command.CommandText = sb.ToString();
					command.CommandType = CommandType.Text;
					
					return FillEntity(command);
				}
			}
		}
	
		[DataObjectMethod(DataObjectMethodType.Select, false)]
		public static Tag GetFirst (string where)
		{
			return GetFirst(where, String.Empty);
		}
	
		[DataObjectMethod(DataObjectMethodType.Select, false)]
		public static Tag GetFirst ()
		{
			return GetFirst(String.Empty, String.Empty);
		}
		
		#endregion
		
		#region Get Latest
		
		[DataObjectMethod(DataObjectMethodType.Select, false)]
		public static Tag GetLatest (string where)
		{
			return GetFirst(where, "ModifiedDT desc");
		}
		
		[DataObjectMethod(DataObjectMethodType.Select, false)]
		public static Tag GetLatest ()
		{
			return GetLatest(String.Empty);
		}
		
		#endregion
		
		#region Get By Foreign Key
		
		#endregion
		
		#region Get By Index
		
		[DataObjectMethod(DataObjectMethodType.Select, false)]
		public static TagCollection GetBySortOrder(int sortOrder, string orderBy)
		{
			return GetList("SortOrder = " + sortOrder + "", orderBy);
		}
		
		[DataObjectMethod(DataObjectMethodType.Select, false)]
		public static TagCollection GetBySortOrder(int sortOrder)
		{
			return GetBySortOrder(sortOrder, String.Empty);
		}
		
		[DataObjectMethod(DataObjectMethodType.Select, false)]
		public static Tag GetByTagID(int tagID, string orderBy)
		{
			return GetFirst("TagID = " + tagID + "", orderBy);
		}
		
		[DataObjectMethod(DataObjectMethodType.Select, false)]
		public static Tag GetByTagID(int tagID)
		{
			return GetByTagID(tagID, String.Empty);
		}
		
		#endregion

		#region Insert
		
		protected static bool InsertOrUpdate (int tagID, string name, string description, int sortOrder)		
		{
			using(SqlConnection connection = new SqlConnection(ConfigurationManager.ConnectionStrings["ManagedFusion"].ConnectionString))
			{
				using (SqlCommand command = connection.CreateCommand())
				{
					command.CommandText = "Journal_Tag";
					command.CommandType = CommandType.StoredProcedure;
					
					command.Parameters.AddWithValue("@TagID", tagID);
					command.Parameters.AddWithValue("@Name", name);
					command.Parameters.AddWithValue("@Description", description);
					command.Parameters.AddWithValue("@SortOrder", sortOrder);
					
					bool success = false;
					
					try
					{
						connection.Open();
						command.ExecuteNonQuery();
					
						success = true;
					} catch (Exception exc) {
						Debug.WriteLine(exc);
						
						success = false;
					} finally {
						connection.Close();
					}
					
					return success;
				}
			}
		}
		
		[DataObjectMethod(DataObjectMethodType.Insert, false)]
		public static bool Insert (int tagID, string name, string description, int sortOrder)
		{
			return InsertOrUpdate(
				tagID,
				name,
				description,
				sortOrder
			);
		}
		
		[DataObjectMethod(DataObjectMethodType.Insert, true)]
		public static bool Insert (Tag entity)
		{
			entity.AcceptChanges();
			return InsertOrUpdate(
				entity.TagID, 
				entity.Name, 
				entity.Description, 
				entity.SortOrder
			);
		}
		
		#endregion
		
		#region Update
		
		[DataObjectMethod(DataObjectMethodType.Update, false)]
		public static bool Update (int tagID, string name, string description, int sortOrder)
		{
			return InsertOrUpdate(
				tagID,
				name,
				description,
				sortOrder
			);
		}
		
		[DataObjectMethod(DataObjectMethodType.Update, true)]
		public static bool Update (Tag entity)
		{
			entity.AcceptChanges();
			return InsertOrUpdate(
				entity.TagID, 
				entity.Name, 
				entity.Description, 
				entity.SortOrder
				);
		}
		
		#endregion
		
		#region Delete
		
		[DataObjectMethod(DataObjectMethodType.Delete, true)]
		public static bool Delete (Tag entity)
		{
			entity.AcceptChanges();
			return Delete(
				entity.TagID
			);
		}

		[DataObjectMethod(DataObjectMethodType.Delete, true)]
		public static bool Delete (int tagID)
		{
			using(SqlConnection connection = new SqlConnection(ConfigurationManager.ConnectionStrings["ManagedFusion"].ConnectionString))
			{
				using (SqlCommand command = connection.CreateCommand())
				{
					command.CommandText = "Journal_Tag_Delete";
					command.CommandType = CommandType.StoredProcedure;
					
					command.Parameters.AddWithValue("@TagID", tagID);
					
					bool success = false;
					
					try
					{
						connection.Open();
						command.ExecuteNonQuery();
					
						success = true;
					} catch (Exception exc) {
						Debug.WriteLine(exc);
						
						success = false;
					} finally {
						connection.Close();
					}
					
					return success;
				}
			}
		}
		
		#endregion

		#endregion
		
		#region Column Variables
		
		#region Primary key(s)
		
		/// <summary>			
		/// Column TagID : 
		/// </summary>
		/// <remarks>Member of the primary key of the underlying table "Tag"</remarks>
		private int _tagID = 0;

		#endregion
		
		#region Non Primary key(s)
		
		/// <summary>
		/// Column Name : 
		/// </summary>
		private string _name = String.Empty;

		/// <summary>
		/// Column Description : 
		/// </summary>
		private string _description = (string)null;

		/// <summary>
		/// Column SortOrder : 
		/// </summary>
		private int _sortOrder = 0;

		#endregion
		
		#endregion
		
		#region Constructor
		
		///<summary>
		/// Creates a new <see cref="Tag"/> instance.
		///</summary>
		///<param name="TagID"></param>
		///<param name="Name"></param>
		///<param name="Description"></param>
		///<param name="SortOrder"></param>
		public Tag (int tagID, string name, string description, int sortOrder)
		{
			this._isMarkedForDeletion = false;
			this._isDirty = true;
			this._isNew = true;
			this._autoUpdate = true;
				
			this._tagID = tagID;
			this._name = name;
			this._description = description;
			this._sortOrder = sortOrder;
		}
		
		public Tag ()
		{
			this._isMarkedForDeletion = false;
			this._isDirty = false;
			this._isNew = true;
			this._autoUpdate = true;
		}
		
		#endregion
		
		#region Properties
		
		#region Foreign Keys
		
		[Browsable(false)]
		public JournalTagLinkCollection ForeignJournalTagLinks
		{
			get 
			{
				return JournalTagLink.GetList("TagID = " + _tagID + "");
			}
		}
		
		#endregion
		
		/// <summary>Gets the TagID value for the column.</summary>
		/// <remarks></remarks>
		/// <value>This type is int</value>
		[ReadOnly(true)]
		[Description("")]
		[DataObjectField(true, true, false, 4)]
		public int TagID
		{
			get { return this._tagID; }
		}
		
		/// <summary>Gets or sets the Name value for the column.</summary>
		/// <remarks></remarks>
		/// <value>This type is varchar</value>
		[Description("")]
		[DataObjectField(false, false, false, 50)]
		public string Name
		{
			get { return this._name; }
			set
			{
				if (_name == value)
					return;
					
				_name = value;
				this._isDirty = true;
				
				// if auto update is turned on update this
				if (AllowAutoUpdate) Update(this);
			}
		}
		
		/// <summary>Gets or sets the Description value for the column.</summary>
		/// <remarks></remarks>
		/// <value>This type is varchar</value>
		[Description("")]
		[DataObjectField(false, false, true, 255)]
		public string Description
		{
			get { return this._description; }
			set
			{
				if (_description == value)
					return;
					
				_description = value;
				this._isDirty = true;
				
				// if auto update is turned on update this
				if (AllowAutoUpdate) Update(this);
			}
		}
		
		/// <summary>Gets or sets the SortOrder value for the column.</summary>
		/// <remarks></remarks>
		/// <value>This type is int</value>
		[Description("")]
		[DataObjectField(false, false, false, 4)]
		public int SortOrder
		{
			get { return this._sortOrder; }
			set
			{
				if (_sortOrder == value)
					return;
					
				_sortOrder = value;
				this._isDirty = true;
				
				// if auto update is turned on update this
				if (AllowAutoUpdate) Update(this);
			}
		}
		

		private bool _autoUpdate = true;
		/// <summary>True if the entity should commit changes as soon as they are made.</summary>
		[Browsable(false)]
		public bool AutoUpdate
		{
			get { return this._autoUpdate; }
			set { _autoUpdate = value; }
		}

		private bool _isMarkedForDeletion = false;
		/// <summary>Gets if the object has been <see cref="MarkToDelete"/>.</summary>
		[Browsable(false)]
		public bool IsMarkedForDeletion
		{
			get { return this._isMarkedForDeletion; }
		}

		private bool _isDirty = false;
		/// <summary>Indicates if the object has been modified from its original state.</summary>
		///<value>True if object has been modified from its original state; otherwise False;</value>
		[Browsable(false)]
		public bool IsDirty
		{
			get { return this._isDirty; }
		}

		private bool _isNew = false;
		/// <summary>Indicates if the object is new.</summary>
		///<value>True if objectis new; otherwise False;</value>
		[Browsable(false)]
		public bool IsNew
		{
			get { return this._isNew; }
		}

		/// <summary>Gets a value indicating if AutoUpdate is allowed on this entity.</summary>
		private bool AllowAutoUpdate 
		{
			get { return (!IsNew && !IsMarkedForDeletion) && AutoUpdate; }
		}

		#endregion
		
		#region Methods
		
		internal void Merge (Tag entity)
		{
			this._tagID = entity._tagID;
			this._name = entity._name;
			this._description = entity._description;
			this._sortOrder = entity._sortOrder;
		}

		/// <summary>Begin the update process.</summary>
		public void BeginUpdate()
		{
			this.AutoUpdate = false;
		}
		
		/// <summary>End the update process and commit changes.</summary>
		public void EndUpdate()
		{
			this.EndUpdate(true);
		}
		
		/// <summary>End the update process</summary>
		public void EndUpdate(bool commit)
		{
			this.AutoUpdate = true;
			
			if (commit)
				this.CommitChanges();
		}
		
		/// <summary>Accepts the changes made to this object by setting each flags to false.</summary>
		internal void AcceptChanges()
		{
			this._isMarkedForDeletion = false;
			this._isDirty = false;
			this._isNew = false;
		}
		
		///<summary>Currently not supported.</summary>
		public void CancelChanges()
		{
			throw new NotSupportedException("Cancel changes is not currently supported.");
		}
		
		///<summary>Delete this entity.</summary>
		public void Delete()
		{
			this._isMarkedForDeletion = true;
			
			if (!IsNew && AutoUpdate) Delete(this);
		}
		
		#endregion
		
		#region ITable<int> Members

		[DataObjectField(true, true, false)]
		int ITable<int>.PrimaryKey
		{
			get { return _tagID; }
		}

		/// <summary>Commit the changes to the database.</summary>
		public void CommitChanges()
		{
			if (this.IsNew)
				Insert(this);
			
			else if (this.IsMarkedForDeletion)
				Delete(this);
				
			else if (this.IsDirty)
				Update(this);
		}

		#endregion
	}
	
	#region Tag Columns
	
	public enum TagColumn
	{
		/// <summary></summary>
		TagID,
 
		/// <summary></summary>
		Name,
 
		/// <summary></summary>
		Description,
 
		/// <summary></summary>
		SortOrder 
	}
	
	#endregion
	
	#region Tag Collection
	
	public class TagCollection : TableCollection<int, Tag>
	{
	}
	
	#endregion
}